...
table(iris_test$Prediction_species)/nrow(iris_test)
table_mat<-table(iris_test$Prediction_Species,iris_test$Species)
table_mat
...
#Accuracy (MAPE)
mape <- mean(abs((iris_test$Sepal.Length-iris_test$Prediction_Sepal.Length ))/iris_test$Prediction_Sepal.Length)
mape
...
rpart.plot(tree_ms5, main = "minsplit=2")
